        ...
        <div className="min-h-screen bg-slate-50/50 flex flex-col md:flex-row">
            ...
            <aside className="w-full md:w-64 bg-slate-900 text-slate-300 flex flex-col border-r border-slate-800 shrink-0">
                <div className="p-6 border-b border-slate-800 flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-indigo-600 flex items-center justify-center text-white font-display font-black text-xl shadow-md">
                        OD
                    </div>
                    <div>
                        <h2 className="font-display font-black text-white leading-tight">Pro Portal</h2>
                        <span className="text-2xs text-indigo-400 font-bold uppercase tracking-wider">Dashboard v1.0</span>
                    </div>
                </div>
                ...
                <nav className="flex-1 p-4 space-y-1.5">
                    <Link 
                        ...
                        className="flex items-center gap-3.5 px-4 py-3 bg-slate-800/80 text-white font-bold rounded-2xl transition-all shadow-sm"
                    >
                        <FaCompass className="text-indigo-400 text-base" />
                        <span className="text-sm">Overview</span>
                    </Link>
                    <Link 
                        to="/professional/professional-bookings" 
                        className="flex items-center gap-3.5 px-4 py-3 hover:bg-slate-800 hover:text-white font-semibold rounded-2xl transition-all text-slate-400"
                    >
                        <FaClipboardList className="text-slate-400 text-base" />
                        <span className="text-sm">My Bookings</span>
                    </Link>
                    <Link 
                        to="/professional/professional-profile" 
                        className="flex items-center gap-3.5 px-4 py-3 hover:bg-slate-800 hover:text-white font-semibold rounded-2xl transition-all text-slate-400"
                    >
                ...
                <div className="p-4 border-t border-slate-800">
                    <div className="flex items-center gap-3 bg-slate-800/40 p-3 rounded-2xl">
                        ...
                        <div className="overflow-hidden">
                            <h4 className="font-bold text-white text-xs truncate">{profile?.name}</h4>
                            <span className="text-2xs text-slate-500 font-bold truncate block capitalize">{categoryName}</span>
                        </div>
                    </div>
                </div>
            </aside>
            ...
            <main className="flex-1 p-6 sm:p-8 max-w-6xl mx-auto w-full space-y-8">
                {/* Welcome Card Banner */}
                <div className="relative bg-white border border-slate-100 rounded-3xl p-6 sm:p-8 shadow-soft overflow-hidden flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
                    ...
                    <div className="flex items-center gap-5 relative z-10">
                        ...
                        <div className="space-y-1">
                            <span className="inline-flex px-2.5 py-0.5 rounded-full text-3xs font-extrabold bg-indigo-50 text-indigo-600 border border-indigo-100/50 uppercase tracking-wider">{categoryName}</span>
                            <h2 className="text-xl sm:text-2xl font-display font-black text-slate-800">Welcome Back, {profile?.name}!</h2>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                    {/* Estimated Earnings */}
                    <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-soft hover:shadow-lg transition-all duration-300 flex items-center gap-5">
                ...
                <div className="bg-white border border-slate-100 rounded-3xl shadow-soft overflow-hidden">
                    <div className="p-6 border-b border-slate-100 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="font-display font-black text-slate-800 text-lg">Recent Booking Requests</h3>
                        </div>
                        <Link 
                            to="/professional/professional-bookings" 
                    ...
                    {bookingsList.length > 0 ? (
                        <div className="overflow-x-auto">
                            <table className="w-full text-left border-collapse text-xs sm:text-sm">
                                <thead>
                                    <tr className="bg-slate-50 border-b border-slate-100 text-slate-400 uppercase tracking-wider font-extrabold text-3xs">
                                ...
                                <tbody>
                                    {bookingsList.slice(0, 5).map((booking) => (
                                        <tr key={booking._id} className="hover:bg-slate-50/50 border-b border-slate-50 transition-colors">
                                            ...
                                            <td className="py-4 px-6">
                                                <span className={clsx(
                                                    "inline-flex items-center px-2.5 py-1 rounded-full text-3xs font-extrabold uppercase tracking-wider border",
