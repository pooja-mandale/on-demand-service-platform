...
import { useGetAllAgencyPlumbersQuery, useGetAllPlumbersQuery } from '../../redux/apis/customerApi';
...
const PlumberPage = () => {
    ...
    const { data: normalPlumbers, isSuccess: isNormalSuccess, isLoading: loadingNormal, isError: errorNormal } = useGetAllPlumbersQuery();
    const { data: agencyPlumbers, isSuccess: isAgencySuccess, isLoading: loadingAgency, isError: errorAgency } = useGetAllAgencyPlumbersQuery();

    // Pagination state
    const [currentPage, setCurrentPage] = useState(1);
    const itemsPerPage = 6; // Number of plumbers per page

    if (loadingNormal || loadingAgency) {
        return <Loader text="Finding available plumbers..." />;
    }
    ...
    const combinedData = [
        ...(isNormalSuccess ? normalPlumbers.map((item) => ({ ...item, type: 'Normal' })) : []),
        ...(isAgencySuccess ? agencyPlumbers.map((item) => ({ ...item, type: 'Agency' })) : []),
    ]
    ...
    return (
        <div className="bg-slate-50/50 min-h-screen pt-28 pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full animate-fade-in">
            <div className="text-center mb-12 space-y-3 animate-fade-in-up">
                <span className="text-sm font-bold text-indigo-600 tracking-wider uppercase">Plumbing Experts</span>
                <h2 className="text-3xl sm:text-4xl font-display font-black text-slate-900">Professional Plumbers</h2>
                <p className="text-slate-500 max-w-2xl mx-auto font-medium">Book reliable independent plumbers or agency professionals in just a few clicks.</p>
            </div>
            ...
            {paginatedData.length > 0 ? (
                                    ...
                                    <div className="flex items-center space-x-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-indigo-600 font-bold flex items-center gap-1 mt-1">
                                                <FiTool className="text-2xs" /> {item.categories || 'Plumbing'}
                                            </p>
                                        </div>
                                    </div>
                                    <span 
                ...
                <div className="bg-white rounded-3xl p-16 text-center border border-slate-100 shadow-soft max-w-xl mx-auto">
                    <p className="text-lg font-bold text-slate-700 mb-2">No Plumbers Found</p>
                    <p className="text-slate-450 text-sm">We currently do not have any registered plumbers in this category.</p>
                </div>
            )}
