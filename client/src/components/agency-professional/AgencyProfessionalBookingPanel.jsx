...
import { useAcceptBookingProfessionalMutation, useGetAgencyProfessionalBookingsQuery, useRejectBookingProfessionalMutation } from '../../redux/apis/bookingApi';
...
const AgencyProfessionalBookingPanel = () => {
    ...
    // Fetch the data using the Redux query hook
    const { data, isLoading, isError, error } = useGetAgencyProfessionalBookingsQuery();
    const [currentPage, setCurrentPage] = useState(1);
    const [totalPages, setTotalPages] = useState(1);

    // Use effect to handle pagination when data is loaded or updated
    useEffect(() => {
        if (data) {
            // Ensure data.result is an array
            const bookingsArray = Object.values(data || {}); // Convert object to array
            setTotalPages(Math.ceil(bookingsArray.length / 2)); // Set total pages for pagination
        }
    }, [data]);
    useEffect(() => {
        if (isSuccess) {
    ...
    // Get the bookings to display for the current page
    const paginatedBookings = data ? Object.values(data || {}).slice((currentPage - 1) * 2, currentPage * 2) : [];
                    ...
                    <tbody>
                        {paginatedBookings.map((booking) => (
                            <tr key={booking._id} className="hover:bg-gray-50">
                                ...
                                <td className="px-4 py-2 border">{booking.customerId?.name || 'N/A'}</td>
                                <td className="px-4 py-2 border">{booking.customerId?.email || 'N/A'}</td>
